...
public class GTUDiningHall extends AppCompatActivity {

    //private getFoodList foodList;
    private final String FOOD_LIST_URL = "http://www.gtu.edu.tr/kategori/2589/0/display.aspx?languageId=1";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_gtudining_hall);
    ...
    private void init() {
        ...
        final ImageView ımageView = findViewById(R.id.GTUDining_image);

        if(InformationSingleton.getInstance().getImgGTUDining() == null){
            ...
            new AsyncTask<Void,Void,Void>(){
                ...
                protected void onPostExecute(Void aVoid) {
                    InformationSingleton.getInstance().setImgGTUDining(img[0]);
            ...
            img[0] = InformationSingleton.getInstance().getImgGTUDining();
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ımageView.setImageBitmap(img[0]);
